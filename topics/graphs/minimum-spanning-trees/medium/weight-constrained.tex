...
algorithm with a queue or stack instead of a priority queue (this would be
equivalent to BFS/DFS). The runtime of this algorithm is in
...
if the weight is 1 and otherwise add it to the second queue (the weight must be
2). When we pop from the priority queue, we take from the first queue, unless
it is empty, in which case we take from the second. The runtime of this
algorithm is in $\Theta(|E|)$ as priority queue operations are now constant
time.
\end{solution}
\end{parts}
\end{blocksection}
