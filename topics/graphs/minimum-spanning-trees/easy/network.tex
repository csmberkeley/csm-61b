...
\part One algorithm to find a minimum spanning tree is Kruskal's algorithm.  Kruskal's algorithm runs as follows: 
\begin{itemize}
 	\item Sort all the edges by increasing order of their weight
	\item Pick the smallest edge and check if it forms a cycle with the spanning tree so far.  If it doesn't form a cycle, add this edge to the spanning tree.  Otherwise, discard it
	\item Repeat the previous step until there are $(|V|-1)$ edges in the spanning tree, where $|V|$ is the number of vertices in the graph.
\end{itemize}
