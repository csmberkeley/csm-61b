...
\texttt{removeMin()}: If the bucket with index counter is non-empty, remove and return the first vertex in its linked list. Otherwise, increment counter until we find a non-empty bucket. If the counter reaches 11, reset it to 0 and continue (so in effect our array is circular).

\texttt{insert()}: Given a vertex $v$ and a distance $d$, insert the vertex into the beginning of the linked list at index $d \mod 11$.
